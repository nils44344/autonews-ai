  ...
  for (const topic of selected) {
    try {
      ...
      if (qa.finalScore >= env.MIN_QUALITY_SCORE) {
        ...
        console.log(`    ✓ published as ${article.type}`);

        // Only a NEWS pillar spawns a supporting blog cluster (explainers don't).
        // Each cluster post is assessed + published too (was the earlier bug).
